...
namespace WebCompiler.Managers
{
    public class CompilerManager : ICompilerManager
    {
        ...
        {
            "program","do", "while", "enddo", "if", "then", "fi", "read", "write", "set", "var", "endprogram", "equals", "greaterthn", "lessthn"
        };
        ...
        public List<LexemInCode> Lexemes { get; } = new List<LexemInCode>();
            ...
            foreach (string symbol in code.Select(c => c.ToString()))
            {
                ...
                if (TrimDelimiters.Contains(symbol) || Delimiters.Contains(symbol))
                {
                    ...
                    else if (IsIdentifier(lexeme))
                    {
                        ...
                        if (Lexemes.Any() && Lexemes.Last().SubString.Equals("var") && Identifiers.Any(e => e.Name.Equals(lexeme)))
                        {
                        ...
                        if (Lexemes.Any() && !Lexemes.Last().SubString.Equals("var") && !Lexemes.Last().SubString.Equals("program") && !Identifiers.Any(e => e.Name.Equals(lexeme)))
                        {
        ...
        private bool ParseProgram(List<LexemInCode> lexemes, ref int i)
        {
            ...
            if (!lexemes[i].Token.Equals("identifier"))
            {
                SyntaxErr.Add(
                    new SyntaxErrors
                    {
                        Text = $"Program name identifier is expected, seen {lexemes[i].SubString} in line {lexemes[i].LineNumber}"
                    });
                return false;
            }
            ...
            var count = lexemes.Count - 1 - i;
            var diff = lexemes.GetRange(i+1, count);
            if (i != lexemes.Count - 1 && diff.Any(e=>!e.Token.Equals("delimiter")))
            {
                SyntaxErr.Add(
                   new SyntaxErrors
                   {
                       Text = $"Smth was found in the end of the program: {lexemes[i].SubString} in line {lexemes[i].LineNumber}"
                   });
                return false;
            }
        ...
        private bool ParseStatementsList(List<LexemInCode> lexemes, ref int i)
        {
            bool? err;
        ...
        private bool? ParseStatement(List<LexemInCode> lexemes, ref int i)
        {
        ...
        private bool ParseDelimiter(List<LexemInCode> lexemes, ref int i)
        {
            if (!lexemes[i].Token.Equals("delimiter"))
            {
                SyntaxErr.Add(
        ...
        private bool ParseLoop(List<LexemInCode> lexemes, ref int i)
        {
            i++;
            ...
            if (!lexemes[i].Token.Equals("while"))
            {
                SyntaxErr.Add(
                  new SyntaxErrors
                  {
                      Text = $"'while' keyword expected in line {lexemes[i].LineNumber} instead seen {lexemes[i].SubString}"
                  });
                return false;
            }
            i++;
            ...
            if (!lexemes[i].SubString.Equals("enddo"))
            {
                SyntaxErr.Add(
                  new SyntaxErrors
                  {
                      Text = $"'enddo' keyword expected in line {lexemes[i].LineNumber} instead seen {lexemes[i].SubString}"
                  });
                return false;
            }
            i++;
        ...
        private bool ParseConditional(List<LexemInCode> lexemes, ref int i)
        {
            i++;
            ...
            if (!lexemes[i].SubString.Equals("then"))
            {
                SyntaxErr.Add(
                     new SyntaxErrors
                     {
                         Text = $"'then' keyword expected in line {lexemes[i].LineNumber} instead seen {lexemes[i].SubString}"
                     });
                return false;
            }
            i++;
            ...
            if (!lexemes[i].SubString.Equals("fi"))
            {
                SyntaxErr.Add(
                    new SyntaxErrors
                    {
                        Text = $"'fi' keyword expected in line {lexemes[i].LineNumber} instead seen {lexemes[i].SubString}"
                    });
                return false;
            }
            i++;
        ...
        private bool ParseInput(List<LexemInCode> lexemes, ref int i)
        {
            i++;
            ...
            if (!lexemes[i].Token.Equals("identifier"))
            {
                SyntaxErr.Add(
                    new SyntaxErrors
                    {
                        Text = $"Identifier expected in line {lexemes[i].LineNumber} instead seen {lexemes[i].SubString}"
                    });
                return false;
            }
            i++;
        ...
        private bool ParseOutput(List<LexemInCode> lexemes, ref int i)
        {
            i++;
        ...
        private bool ParseDeclaration(List<LexemInCode> lexemes, ref int i)
        {
            i++;
            ...
            if (lexemes[i].Token.Equals("set"))
            {
                ...
                if (!ParseArthExpression(lexemes, ref i))
                {
                    ...
                    new SyntaxErrors
                    {
                        Text = $"'set' keyword expected in line {lexemes[i].LineNumber} instead seen {lexemes[i].SubString}"
                    });
                    return false;
                }
            }
        ...
        private bool ParseAssign(List<LexemInCode> lexemes, ref int i)
        {
            i++;
            if (!lexemes[i].Token.Equals("set"))
            {
        ...
        private bool ParseLogicalExpression(List<LexemInCode> lexemes, ref int i)
        {
            ...
            if (!lexemes[i].Token.Equals("equals") && !lexemes[i].Token.Equals("greaterthn") && !lexemes[i].Token.Equals("lessthn"))
            {
                SyntaxErr.Add(
                    new SyntaxErrors
                    {
                        Text = $"'equals' or 'greaterthn' or 'lessthn' keyword expected in line {lexemes[i].LineNumber} instead seen {lexemes[i].SubString}"
                    });
                return false;
            }
            i++;
        ...
        private bool ParseArthExpression(List<LexemInCode> lexemes, ref int i)
        {
            ...
            var leaf = ParseArithmeticLeaf(lexemes, ref i);
            if (!parenth && !leaf)
                return false;
            //i--;
            ...
            if (!parenth)
            {
                var iBeforeOperation = i;
                var operation = ParseOperationLight(lexemes, ref i);
                if (!operation)
                    return true;
                else
                {
        ...
        private void ParseSign(List<LexemInCode> lexemes, ref int i)
        {
        ...
        private bool ParseArithmeticLeaf(List<LexemInCode> lexemes, ref int i)
        {
            if (lexemes[i].Token.Equals("float") || lexemes[i].Token.Equals("integer") || lexemes[i].Token.Equals("identifier"))
            {
                i++;
                return true;
            }
        ...
        private bool ParseArthExpressionWithOperation(List<LexemInCode> lexemes, ref int i)
        {
        ...
        private bool ParseOperation(List<LexemInCode> lexemes, ref int i)
        {
            if (!lexemes[i].SubString.Equals("+") && !lexemes[i].SubString.Equals("-") && !lexemes[i].SubString.Equals("*") & !lexemes[i].SubString.Equals("/"))
            {
                SyntaxErr.Add(
                    new SyntaxErrors
                    {
        ...
        private bool ParseOperationLight(List<LexemInCode> lexemes, ref int i)
        {
            if (!lexemes[i].SubString.Equals("+") && !lexemes[i].SubString.Equals("-") && !lexemes[i].SubString.Equals("*") & !lexemes[i].SubString.Equals("/"))
            {
                return false;
